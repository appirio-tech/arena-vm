...
//                int index = 0;
    ...
    public ProblemComponent getProblemComponent(int componentId, boolean unsafe) {
    	return getProblemComponent(componentId, unsafe, true, true);
    }
    
    public ProblemComponent getProblemComponent(int componentId, boolean unsafe, boolean loadTests, boolean loadCategories) {
        PreparedStatement ps = null;
            ...
            try {
                ...
                if (pastProposal) {
                    ...
                    for(int i = 0; i < expectedValues.length;i++)
                    {
                        ...
                        try {
                            tresults = getMPSQASServices().test((Object[]) args[i], userId, componentId,
            ...
            try {
                ...
                if (unknownCases.length > 0) {
                    s_trace.debug("Filling test case output of " + unknownCases.length +
                            " cases.");
                    fillTestCasesOutput(unknownCases, userId, component.getComponentId());
                }
    ...
    public ProblemComponent parseProblemStatement(String xml, boolean unsafe,
        ...
        try {
            ...
            if (unknownCases.length > 0) {
                s_trace.debug("Filling test case output of " + unknownCases.length +
                        " cases.");
                // TODO: not sure an easy way to get userId here
                fillTestCasesOutput(unknownCases, 0, problemComponent.getComponentId());
            }
...
//    }
    ...
    private com.topcoder.shared.problem.TestCase[] fillTestCasesOutput(
            com.topcoder.shared.problem.TestCase[] testCases, int userId, int componentId) {
            ...
            if (solutionId != -1) {
                ...
                if (argsToTest.size() > 0) {
                    ...
                    MPSQASFiles tresults[] = getMPSQASServices().test(allArgValues,
                            userId, componentId, solutionId, methodName);
...
//        private Connection conn = null;
        ...
        MPSQASTesterThread(int num, Object args, int userId, int solutionId, int componentId, Connection conn, String methodName)
        {
            done = false;
            this.num = num;
            this.args = args;
...
//            this.conn = conn;
        ...
        {
            try {
                tresults = getMPSQASServices().test((Object[]) args, userId, componentId,
                                primarySolutionId, "checkData");
            } catch (Exception e) {
                e.printStackTrace();

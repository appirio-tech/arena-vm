 ...
 */
public class PhaseListField extends DropDownField {
    ...
    private static final Integer[] PHASES;
     ...
     */
    static {
        PHASES = new Integer[ContestConstants.PHASES.length];
        for (int i = 0; i < ContestConstants.PHASES.length; i++) {
            PHASES[i] = new Integer(ContestConstants.PHASES[i]);
        }
    }
     ...
     * @see    ContestConstants.PHASE_NAMES
     */
    public PhaseListField(int index) {
        super(ContestConstants.PHASE_NAMES);

        if (index < NOT_SELECTED || index >= ContestConstants.PHASE_NAMES.length) {

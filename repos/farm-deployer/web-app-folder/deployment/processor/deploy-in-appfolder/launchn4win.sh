LAUNCH=c:/processor/launcher
CACHE=$LAUNCH/cache
DEPLOY=c:/processor/deploy

while :
do
    ID=$(cat $LAUNCH/idfile)
    LOGFILE=$LAUNCH/processor-`date +%Y-%m-%d-%H-%M-%S`.log
    URL=http\://farm-controller.topcoder.com\:8080/farm-deployer/launch.jnlp\?type=processor\&id=$ID\&os_type=windows\&os_version=64bit

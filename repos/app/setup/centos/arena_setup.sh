...
function build_application() {
    ...
    cp ~/dev/app/token.properties.ci ~/dev/app/token.properties
...
function clean_install_dirs() {
    rm -rf ~/jboss > /dev/null 2>&1
    rm -rf ~/jboss-4.0.5.GA > /dev/null 2>&1
    rm -rf ~/tomcat > /dev/null 2>&1
    rm -rf ~/apache-tomcat-5.5.27 > /dev/null 2>&1
    rm -rf ~/controller > /dev/null 2>&1
    rm -rf ~/processor > /dev/null 2>&1
    rm -rf ~/deployer > /dev/null 2>&1
}

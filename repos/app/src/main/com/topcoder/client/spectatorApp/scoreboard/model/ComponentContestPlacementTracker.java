...
public class ComponentContestPlacementTracker implements CompetitionPlacementTracker<ComponentCoder> {
	
	/** Logging support */
	private static final Category cat = Category.getInstance(ComponentContestPlacementTracker.class.getName());

	/** Related tracker */
	private final ComponentContest tracker;
	...
	/** Coders in placement order (ie placement of 1 [index 0] is coder index blah blah */
	...
	/** Create the tracker */
	ComponentContestPlacementTracker(ComponentContest tracker)
	{
		...
		for(int x = 0; x < coderMax; x++) {
			...
			scores[x] = tracker.getTotalScore(idx);
			coderPlacement[x] = x;
			placementOrder[x] = x;
		}
	...
	public synchronized boolean isTied(int coderID)
	{
		...
		if (idx < 0) {
			...
			else if (idx < coderPlacement.length - 1 && coderPlacement[idx] == coderPlacement[idx + 1]) return true;
			return false;
		}
	}
	...
	private boolean calculateSimplePlacement() {
		...
		for (int place = 0; place < tempScores.length; place++) {
			// Reset the 'high' coder
			int newHigh = -1;
			ComponentCoder highCoder = null;
	...
	{
		changeSupport.fireUpdatePlacement(new PlacementChangeEvent(ComponentContestPlacementTracker.this));
	}
	...
	private class ShowPlacementHandler implements ShowPlacementListener
	{
		...
		public void showTCSPlacements(ShowTCSPlacementEvent evt) {
			synchronized(ComponentContestPlacementTracker.this) {

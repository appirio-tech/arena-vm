...
public class SubmissionCountPanel extends DirectDrawPanel implements AnimatePanel {
	...
	public SubmissionCountPanel(ScoreboardPointTracker pointTracker, int coderID, int problemID, boolean example) {
		...
		// Set the size
		super.setSize(pointFontFM.stringWidth(formatter.format(9999)), pointFontFM.getAscent());
		
		// Set the vertical alighment to the ascent
		setVerticalAlignment(pointFontFM.getAscent());
                
                if(!example)
                    priorValue = pointTracker.getSubmissionCount(row, col);
                else
                    priorValue = pointTracker.getExampleCount(row, col);
	}
	...
	public void drawImage(Graphics2D g2D) {
		...
		int pointValue = 0;
                if(!example)
                    pointValue = pointTracker.getSubmissionCount(row, col);
                else 
                    pointValue = pointTracker.getExampleCount(row, col);

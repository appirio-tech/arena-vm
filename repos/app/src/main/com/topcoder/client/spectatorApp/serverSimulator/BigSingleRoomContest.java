...
public class BigSingleRoomContest extends Contest {
	...
	public BigSingleRoomContest() {}

	public BigSingleRoomContest(SimulatorProcessor processor) {
	...
	class Room3 extends Room {
		...
		public Room3(int roomID, int roomType, String roomTitle, int roundID) {
			...
			Coder c5 = new Coder(5, "TheFaxmansdfsdf", 5, 5);
			...
			resultList.add(c3.scheduleCoding(roomData, p3, 60, 50, true));
			...
			c7.scheduleChallenge(roomData, p3, c8, 30, 8, 1);
			c7.scheduleChallenge(roomData, p1, c5, 30, 8, -1);
	...
	class Coder {
		...
		public TheResult scheduleCoding(RoomData roomData, ProblemID problemID, int time, int elapsed, boolean good) {
			...
			double value = 0;
			if (elapsed > 0) {
				value = calcTotal(problemID.getValue(), elapsed, COD) / 100.;

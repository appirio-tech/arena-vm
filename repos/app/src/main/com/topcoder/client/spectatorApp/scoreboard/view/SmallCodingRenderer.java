...
public class SmallCodingRenderer extends SPage {
	/** Reference to the logging category */
//	private static final Category cat = Category.getInstance(SmallCodingRenderer.class.getName());

	/** Font used for "Point Value" title */
	private static final Font pointValueTitleFont = FontFactory.getInstance().getFont(FontFactory.FUTURA_BOLD,
				Font.PLAIN, 18);
	...
	public SmallCodingRenderer(String roomTitle, String contestName, ScoreboardPointTracker pointTracker) {
		super(new HeaderPanel(roomTitle), new CodingRenderer(pointTracker), new TrailerPanel(contestName));
	}

	static class CodingRenderer extends LayeredPanel {
		/** The title layer */
		...
		public CodingRenderer(ScoreboardPointTracker pointTracker) {
			// Create the table
			...
			for (int r = 0; r < rowSize; r++) {
				...
				cells[r][cells[r].length - 1] = new SGridCell(totalPanels[r], SGridCell.VERTICAL_ALIGNMENT_EAST,
							SGridCell.NONE);
				rowHeight = Math.max(rowHeight, cells[r][cells[r].length - 1].getHeight());
			}
			
			// Setup the policies
			table.setRowSpacingPolicy(new SGridSpacingPolicy.HeaderSpacingPolicy(10,
						new SGridSpacingPolicy.EdgePercentageSpacingPolicy(1.0, .00)));

 ...
 *
 * <p>
 * Changes in version 1.1 (TopCoder Competition Engine - Automatically End Matches v1.0) :
 * <ol>
 *         <li>Update {@link #processNonLongRound(Timer timer, boolean sendPastEvent, Round round, Date currentTime,
 *                Date regStart, Date regEnd, Date assignStart, Date assignEnd, Date codingStart, Date codingEnd,
 *                Date challengeStart, Date challengeEnd)} method.</li>
 * </ol>
 * </p>
 * @author TCDEVELOPER, TCSASSEMBLER
 * @version 1.1
 */
public final class RoundTimerTaskSet {
    ...
    /**
     * Process the time task with SRM
     * @param timer the timer.
     * @param sendPastEvent if it is needed send past event.
     * @param round the round data.
     * @param currentTime the current time
     * @param regStart the registration start time.
     * @param regEnd the registration end time.
     * @param assignStart the room assignment start time.
     * @param assignEnd the room assignment end time.
     * @param codingStart the coding start time.
     * @param codingEnd the coding end time.
     * @param challengeStart the challenge start time.
     * @param challengeEnd the challenge end time.
     */
    private void processNonLongRound(Timer timer, boolean sendPastEvent, Round round, Date currentTime, Date regStart,
            Date regEnd, Date assignStart, Date assignEnd, Date codingStart, Date codingEnd, Date challengeStart,
            Date challengeEnd) {
        if(sendPastEvent) {
            if (currentTime.before(regStart)) {
                timer.schedule(countDownStartTask, currentTime); // fire right away
            ...
            if (currentTime.before(challengeEnd)) {
                ...
                if (round.isAutoEnd()) {
                    systemTestStartTask = new StartSystemTestsTimerTask((int) roundID, true, true);
                    timer.schedule(systemTestStartTask, new Date(challengeEnd.getTime() + 1));    
                }
            }
    ...
    /**
     * The start system tests timer task.
     * <p>
     * Changes in version 1.1 (TopCoder Competiton Engine - Automatically End Matches v1.0) :
     * <ol>
     *         <li>Add {@link #isFailOnFirstBadTest} field.</li>
     *         <li>Add {@link #isEndAutomatically} field.</li>
     *         <li>Add {@link #StartSystemTestsTimerTask(int roundId, boolean isFailOnFirstBadTest,
     *                 boolean isEndAutomatically)} method.</li>
     *         <li>Update {@link #run()} method.</li>
     * </ol>
     * </p>
     * @author TCSASSEMBLER
     * @version 1.1
    ...
    private static class StartSystemTestsTimerTask extends TimerTask {
        private int roundId;
        /**
         * If it is needed to fail on the first bad test.
         * @since 1.1
         */
        private boolean isFailOnFirstBadTest;
        /**
         * If the match is ended automatically.
         * @since 1.1
         */
        private boolean isEndAutomatically;
         ...
         * @param isFailOnFirstBadTest if it is needed to fail on the first bad test.
         * @param isEndAutomatically  if the match is ended automatically.
         */
        public StartSystemTestsTimerTask(int roundId, boolean isFailOnFirstBadTest, boolean isEndAutomatically) {
            this.roundId = roundId;
            this.isFailOnFirstBadTest = isFailOnFirstBadTest;
            this.isEndAutomatically = isEndAutomatically;
        }
        /**
         * The run method.
         */
        public void run() {
            ...
            try {
                ...
                AdminCommands.systemTest(roundId, 0, 0, isFailOnFirstBadTest, false, !isEndAutomatically);
                Round contest = CoreServices.getContestRound(roundId);
                contest.setSystemTestEnd(true);
                Processor.processRoundEvent(contest.getContestID(), roundId, ContestConstants.SYSTEM_TESTING_PHASE);
                //After system test, we can end the match automatically
                if (isEndAutomatically) {

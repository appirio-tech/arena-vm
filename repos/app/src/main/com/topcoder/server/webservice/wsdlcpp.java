...
public class wsdlcpp {
    ...
    * definitions node*/
    ...
    public void setNameSpaces(Node node //get the document node
            ) {
        String wsdlNS = null;
        String xsdNS = null;
    ...
    public void myParser(File inputFile, File outputDirectory) throws ParserConfigurationException {
        System.out.println("/*Parsing XML File: " + inputFile + "*/");
        ...
        try {
            ...
            /*write .h file*/
            File outHeaderFile = new File(outputDirectory, inputFile.getName().substring(0, inputFile.getName().indexOf('.')) + ".h");
            File outCFile = new File(outputDirectory, inputFile.getName().substring(0, inputFile.getName().indexOf('.')) + ".c");
            PrintWriter pw = new PrintWriter(new FileWriter(outHeaderFile));
            System.out.println("Processing Done.\n\t\t Header File\t\t" + outHeaderFile.getPath());
            ...
            /*write .c file*/
            pw = new PrintWriter(new FileWriter(outCFile));
            System.out.println("\t\t Sample C File\t\t" + outCFile.getPath());
...
//            }
        ...
        if (soapAddrNode == null) {
            if (defSoap)
                soapAddrNode = (new NodeSearch(serviceNode, "address")).getNextNode();
            if (soapAddrNode == null) {
...
//            }
                ...
                if (tnsp != null) {
                    if (bindingsList.containsKey(tnsp))
                        ...
                        if (!bindingsList.containsValue(tdefns)) {
                            ...
                            int cnt = 1;
                            while (bindingsList.containsValue(tdefns + cnt))
                                cnt++;
    ...
    public void printOperationInfo(Node node, String bindingType, String operationName, String defns, String soapact) {
        ...
        NodeSearch operations = new NodeSearch(new NodeSearch(node, docNS + "portType", bindattrib).getNextNode(), docNS + "operation", operattrib);
        Node operation = operations.getNextNode();
        if ((operation == null) && (defWsdl)) {
            operations = new NodeSearch(new NodeSearch(node, "portType", bindattrib).getNextNode(), "operation", operattrib);
        ...
        while (operation != null) {
            ...
            for (int i = 0; i < input.getTotalMatches(); i++) {
                if (inputNode == null) {
                    System.out.println("INput node for operation " + convertToCpp(operationName, true) + " Not found");
                    return;
                }
            ...
            for (int i = 0; i < output.getTotalMatches(); i++) {
                if (outputNode == null) {
                    System.out.println("outputput node for operation " + convertToCpp(operationName, true) + " Not found");
                    return;
                }
      ...
      */
            String description = "";
            if (!elementflag) {
                description = " ( " + inputArgs.substring(0, inputArgs.length() - 2) + " struct " + dataType(defns + operationName) + "Response {" + outArgs.substring(0, outArgs.length()) + " } *out) ;";
                addOperation(defns, operationName, description, soapact);
            } else {
                description = " ( " + inputArgs.substring(0, inputArgs.length() - 2) + " struct " + dataType(defns + operationName) + "_Response {" + outArgs.substring(0, outArgs.length()) + " } *out) ;";
    ...
    void addOperation(String ldefNS, String operationName, String description, String soapaction) {
    ...
    String getMessageEquivalent(Node node, String messageName, boolean isreturn) {
        ...
        if (nmessageNode != null)
            ...
            System.out.println("message of message name" + messageName + "not found");
            System.exit(0);
        }
        if (messageNode == null) {
            System.out.println("message of message name" + messageName + "not found");
    ...
    * soap encoding*/
    ...
    private int getArrayDimension(String atype) {
        ...
        int cnt = 0;
        int indx = -1;
        while ((indx = last.indexOf(',', indx + 1)) != -1)
            cnt++;
        return cnt + 1;
    }
    ...
    private boolean singleLevelArray(String atype) {
        int cnt = 0;
        int indx = -1;
        while ((indx = atype.indexOf('[', indx + 1)) != -1)
            cnt++;
        if (cnt == 1)
            return true;
        else
    ...
    public static void main(String[] args) throws Exception {
        if (args.length != 2) {
            System.out.println("Usage: java wsdlcpp [XML URI] [Output Directory]");
            System.exit(0);
        }
        wsdlcpp parserParser = new wsdlcpp();
        parserParser.myParser(new File(args[0]), new File(args[1]));
    }
}
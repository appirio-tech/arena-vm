...
public class NsNodeSearch {
    ...
    public NsNode getNextPartialMatch() {
        if (firsttime) {
            firsttime = false;
            if (startNode == null || searchString == null)
                return null;
    ...
    public void findAllElementPartialMatchNode(Node node, String name, Hashtable ns) {
        ...
        if (nodes != null) {
            for (int i = 0; i < nodes.getLength(); i++)
                if (nodes.item(i).getNodeType() == Node.ELEMENT_NODE)
                    findAllElementPartialMatchNode(nodes.item(i), name, new Hashtable(ns));
        }
    }

    public void findAllElementPartialMatchNode(Node node, String name, Hashtable attributes, Hashtable ns) //vector will contain attribute in even position and values in odd position
    {
        String value;
        String key;
        String gotValue;
        Node rnode = null;
        Enumeration keyList;
        if (node == null || name == null || attributes == null)
            return;
        ns = setNameSpaces(node, ns);
    ...
    public void findAllElementNode(Node node, String name, Hashtable attributes, Hashtable ns) //vector will contain attribute in even position and values in odd position
    {
        String value;
        String key;
        String gotValue;

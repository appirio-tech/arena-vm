...
/**
 * This class is responsible for testing long submission and solutions.
 * <p>
 * 
 * Farm interaction and processor selection is resolved by this class.
 * <p>
 * 
 ...
 * {@link #doTestsOnSubmission(LongSubmissionId, ComponentFiles, long[], int, int, int, int, HierarchicalIdBuilder, int)}
 * .</li>
 * </ul>
 * </p>
 * 
 * <p>
 * Changes in version 1.2 (TC Competition Engine - CPP Language Upgrade And
 * Customization Support v1.0):
 * <ol>
 ...
 * 
 * <p>
 * Changes in version 1.3 (TC Competition Engine - Python Language Upgrade And
 * Customization Support v1.0):
 * <ol>
 * <li>Added parameter <code>pythonCommand</code>,
 * <code>pythonApprovedPath</code> to methods
 ...
 * {@link #doTestsOnSubmission(LongSubmissionId, ComponentFiles, long[], int, int, int, String, String, String ,int, HierarchicalIdBuilder, int)}
 * .</li>
 * </ol>
 * </p>
 * 
 * <p>
 * Changes in version 1.4 (TopCoder Competition Engine Improvement Series 3
 * v1.0):
 * <ol>
 ...
 * {@link #doTestsOnSubmission(LongSubmissionId, ComponentFiles, long[], int, ProblemCustomSettings ,int, HierarchicalIdBuilder, int)}
 * .</li>
 * </ol>
 * </p>
 * 
...
public class LongTesterInvoker {
	...
	// TODO we should split this class into one for long submission testing, and
	// another for MPSQAS long testing
	/**
	 ...
	 * R{roundId}.A{testAction}.U{coderId}.S{(1|0
	 * example)(submissionNumber)}.T{testCaseId}..
	 * 
	 ...
	 * A7.A7.G{groupId}.A7.T{testCaseId}.. //Uggly but until split class we need
	 * some pattern
	 */
	 ...
	 * 
	 * Submission Test R{roundId}.A10.U{coderId}
	 * 
	 * System Test for round R{roundId}.A12
	 * 
	 * MPSQAS test for solution A7.A7.G{groupId}.A7
	 */
	 ...
	 * 
	 * UserComponentFilesKey = {prefix}.U{coderId}.S{(1|0
	 * example)(submissionNumber)}..
	 */
...
//	private static final int ACTION_SCORE = 13;
	...
	/**
	 * The tester instance id. This id is used as suffix for the farm client
	 * identifier.
	 */
	...
	/**
	 * Creates a new LongTester and configures it.
	 * <p>
	 * 
	 * NOTE: It is not allowed to create multiple instances of a LongTester
	 * using the same tester id suffix. Doing this will produce unpredictable
	 * results. It client user responsability to ensure that only one instance
	 * for ths given Id is created.
	 * 
	 * @param id
	 *            The id suffix to use for farm identification
	 * @param handler
	 *            The handler that will handle tests results
	 * 
...
//		farmFactory.configureInvoker(testerName, invokerConfiguration);
	...
	/**
	 * Tests the submission against all given testCases .
	 * <p>
	 * 
	 * Test cases must be stored on the farm previously. To store TestCases for
	 * a submission test see
	 * {@link LongTesterInvoker#storeTestCase(LongSubmissionId, long, Object[])}
	 * ;
	 * 
	 * Results of the tests are reported through
	 * {@link LongTesterHandler#reportLongTestSubmissionResult(LongSubmissionId, long, LongTestResults)}
	 * 
	 * @param submissionId
	 *            The id of the submission
	 * @param componentFiles
	 *            The component files of the submission to test
	 * @param primarySolution
	 *            The primary solution required to test the submission.
	 * @param testCaseIds
	 *            The test cases identifiers to run against the submission
	 * @param roundType
	 *            The round type where the submission is being tested
	 * @param custom
	 *            problem customization.
	 * 
	 * @throws LongTesterException
	 *             If the one of the test could not be scheduled
	 */
	public void testSubmission(LongSubmissionId submissionId, ComponentFiles componentFiles, Solution primarySolution,
			long[] testCaseIds, int roundType, ProblemCustomSettings custom) throws LongTesterException {
		...
		doTestsOnSubmission(submissionId, componentFiles, testCaseIds, roundType, custom,
				ServicesConstants.LONG_TEST_ACTION, primarySolution, prefix, getTestSubmissionPriority());
	}

	/**
	 * System test the submission against all test cases given.
	 * <p>
	 * 
	 * Test cases must be stored on the farm previously. To store system tests
	 * cases for a submission system test see
	 * {@link LongTesterInvoker#storeTestCase(int, long, Object[])}. Also,
	 * primary solution should be stored previously using the method
	 * {@link LongTesterInvoker#storePrimarySolution(int, Solution)}.
	 * <p>
	 * 
	 * If there are system test cases for the same submission pending for
	 * execution, the last stored component files will be used.
	 * <p>
	 * 
	 * Results of the tests are reported through
	 * {@link LongTesterHandler#reportLongSystemTestSubmissionResult(LongSubmissionId, long, LongTestResults)}
	 * 
	 * @param submissionId
	 *            The id of the submission
	 * @param componentFiles
	...
	public void systemTestSubmission(LongSubmissionId id, ComponentFiles componentFiles, long[] testCaseIds,
			int roundType, ProblemCustomSettings custom, Solution solution) throws LongTesterException {
		doTestsOnSubmission(id, componentFiles, testCaseIds, roundType, custom,
				ServicesConstants.LONG_SYSTEM_TEST_ACTION, solution,
				buildRoundSystemTestRelatedPrefix(id.getRoundId()), getSystemTestSubmissionPriority());
	}

	/**
	 * Tests the solution against all given testCases .
	 * <p>
	 * 
	 * Results of the tests are reported through
	 * {@link LongTesterHandler#reportLongTestSolutionResult(int, long, LongTestResults)}
	 * 
	 * @param groupId
	 *            The id of the group generated to test the solution
	 * @param componentFiles
	 *            The component files of the solution to test
	 * @param primarySolution
	 *            The primary solution required to test the solution.
	 * @param testCaseIds
	 *            The test cases identifiers generated to tests the solution
	 * @param args
	 *            An array containing an Object[] with args of each test case
	 * @param roundType
	 *            The round type where the submission is being tested, null if
	 *            no roundType is used
	 * @param custom
	 *            problem customization.
	 * 
	 * @throws LongTesterException
	...
	public void testSolution(int groupId, ComponentFiles componentFiles, Solution primarySolution, long[] testCaseIds,
		...
		// InvocationRequirements requirements = buildRequeriments(roundType,
		// memLimit, componentFiles.getLanguageId());
		...
		// may need to store in s3 if too big for a single SQS message
		...
		try {
			...
			// ArrayList<MarathonCodeTestRequest> requests = new
			// ArrayList<MarathonCodeTestRequest>(testCaseIds.length);
			...
			for (int i = 0; i < testCaseIds.length; i++) {
				...
				// requests.add(new MarathonCodeTestRequest(longTest,
				// componentFiles, primarySolution));
				MarathonCodeTestRequest mRequest = new MarathonCodeTestRequest(longTest, componentFiles,
						primarySolution);

				// LongTestInvocation invocation = new
				// LongTestInvocation(longTest);
				LongTestId attachId = new LongTestId(FarmLongTestRequest.CODE_TYPE_SOLUTION, new Integer(groupId),
						testCaseId, ServicesConstants.MPSQAS_TEST_ACTION);
				InvocationRequest request = new InvocationRequest(context.buildId(TYPE_TEST, testCaseId), attachId,
						null, null);

				InvocationData data = controller.storeInvocationRequest(testerId, request);

				String requestTag = data.getId().toString();
				queueTestRequest(mRequest, requestTag, attachId, null,
						CodeUtil.toLanguageType(componentFiles.getLanguageId()));

				// request.addSharedObjectRef("solution", solutionKey);
				// request.addSharedObjectRef("componentFiles",
				// componentFilesKey);
				// getInvoker().scheduleInvocation(request);
			}

			// queueTestRequests(requests, context.getPrefix());
	...
	private void queueTestRequest(MarathonCodeTestRequest testRequest, String requestTag, Object invocationData,
			Integer roundId, LanguageType lang) throws Exception {
		CodeProcessingRequest cpr = new CodeProcessingRequest(testRequest, requestTag, AppType.MARATHON,
				ActionType.TEST, lang, "marathonTester", MarathonCodeProcessingResultHandler.class.getName(),
				roundId, invocationData);
	...
	/**
	 * Calculates the overall scores in the context of a long submission test
	 * finalization.
	 * <p>
	 * 
	 * Results of the score calculation is reported through
	 * {@link LongTesterHandler#reportCalculateTestScoreResult(LongSubmissionId, LongRoundScores)}
	 * 
	 * @param id
	 *            The id of the solution finalized
	 * @param solution
	 *            The primary solution that will be used to calculate the scores
	 * @param longTestResults
	 *            The LongRoundScores used for overall score calculation
	 * @param roundType
	 *            the round type for the component, which is used for selecting
	 *            test processor
	 * 
	 * @throws LongTesterException
	 *             If overall scores calculation not be scheduled
	 */
	public void calculateTestScores(LongSubmissionId id, Solution solution, LongRoundScores longTestResults,
			int roundType) throws LongTesterException {
		doCalculateScores(id.getRoundId(), id, solution, longTestResults, ServicesConstants.LONG_TEST_ACTION, roundType);
	}

	/**
	 * Calculates the overall scores in the context of a round system test
	 * finalization.
	 * <p>
	 * 
	 * Results of the score calculation is reported through
	 * {@link LongTesterHandler#reportCalculateSystemTestScoreSubmissionResult(int, LongRoundScores)}
	 * 
	 * @param roundId
	 *            The id of the round
	 * @param solution
	 ...
	 *             If overall scores calculation not be scheduled
	 */
	public void calculateSystemTestScores(int roundId, Solution solution, LongRoundScores longTestResults, int roundType)
			throws LongTesterException {
		doCalculateScores(roundId, new Integer(roundId), solution, longTestResults,
				ServicesConstants.LONG_SYSTEM_TEST_ACTION, roundType);
	}
	 ...
	 * @throws LongTesterException
	 *             If was not possible to obtain the status
	 */
	public boolean isQueueEmptyForUser(int roundId, long coderId) throws LongTesterException {
		String prefix = buildUserTestRelatedPrefix(roundId, coderId).getPrefix();
		try {
			return isQueueEmptyForRequestTag(prefix);
		} catch (Exception e) {
			throw new LongTesterException(e);
		}
	}

	private boolean isQueueEmptyForRequestTag(String tag) throws Exception {
		return controller.countPendingRequests(testerId, tag).intValue() == 0;
	}
	 ...
	 * 
	 * @throws LongTesterException
	 *             If was not possible to cancel all tests for the user
	 */
	public void cancelTestsForUser(int roundId, long coderId) throws LongTesterException {
		String prefix = buildUserTestRelatedPrefix(roundId, coderId).getPrefix();
		try {
			cancelTests(prefix, roundId);
			controller.removeSharedObjects(testerId, prefix);
		} catch (Exception e) {
			throw new LongTesterException(e);
		}
	}

	private void cancelTests(String prefix, Integer roundId) {
		codeService.deletePendingCodeRequests(testerId, new CodeProcessingRequestMetadata(AppType.MARATHON,
				ActionType.TEST, null, prefix, roundId));
	}
	 ...
	 *             If was not possible to obtain the status
	 */
	// public boolean isEmptyOfSystemTestsForRound(int roundId) throws
	// LongTesterException {
	// String requestId =
	// buildRoundSystemTestRelatedPrefix(roundId).getPrefix();
	// try {
	...
	/**
	 * Returns true if there are not enqueued system test for the the given user
	 * and round
	 * 
	 * @param roundId
	 *            The id of round
	 ...
	 *             If was not possible to obtain the status
	 */
	// public boolean isEmptyOfSystemTestsForUser(int roundId, long coderId)
	// throws LongTesterException {
	// String requestId =
	// buildRequestIdPrefix(buildRoundSystemTestRelatedPrefix(roundId),
	// coderId).getPrefix();
	// try {
	// return getInvoker().countPendingRequests(requestId).intValue() == 0;
	// } catch (Exception e) {
	...
	//
	// /**
	// * Returns true if there are not enqueued items for testing for the round
	// (user or system)
	// *
	...
	public boolean isQueueEmptyForRound(int roundId) throws LongTesterException {
		String tag = buildRoundRelatedPrefix(roundId).getPrefix();
		try {
			return isQueueEmptyForRequestTag(tag);
		} catch (Exception e) {
			throw new LongTesterException(e);
		}
	}
	...
	//
	// /**
	// * Cancels and remove from the Test queue all system test cases enqueued
	// for the round
	// *
	// * @param roundId The id of the round
	// */
	public void cancelSystemTestsForRound(int roundId) throws LongTesterException {
		String tag = buildRoundSystemTestRelatedPrefix(roundId).getPrefix();
		try {
			cancelTests(tag, roundId);
		} catch (Exception e) {
			throw new LongTesterException(e);
		}
	}
	...
	//
	// /**
	// * Cancels and remove from the Test queue all test cases (system and user)
	// enqueued for the round
	// *
	// * @param roundId The id of the round
	// */
	 ...
	 * 
	 * @throws LongTesterException
	 *             If was not possible to cancel all tests for the group
	 */
	public void cancelTestsForGroup(int testGroupId) throws LongTesterException {
		String tag = buildGroupRelatedPrefix(testGroupId).getPrefix();
		try {
			cancelTests(tag, null);
		} catch (Exception e) {
			throw new LongTesterException(e);
		}
	}
	...
	/**
	 * Returns the current queue status for this LongTesterInvoker
	 * <p>
	 * 
	 * Information is summarized in the followinf way: Submission tests are
	 * grouped by testAction, roundId, coderId, example Solution tests are
	 * grouped by testAction, groupId
	 * 
	...
	public List queueStatus() throws LongTesterException {
		try {
			// List enqueuedRequestsSummary =
			// getInvoker().getClientNode().getEnqueuedRequestsSummary("",
			// ""+HierarchicalIdBuilder.DELIMITER, 4);
			List enqueuedRequestsSummary = controller.getEnqueuedRequestsSummary(testerId, "", ""
					+ HierarchicalIdBuilder.DELIMITER, 4);
			...
			for (Iterator it = enqueuedRequestsSummary.iterator(); it.hasNext();) {
				...
				if (disassembler.getValueAsInt(1) == ServicesConstants.MPSQAS_TEST_ACTION) {
					result.add(new LongTesterSummaryItem(disassembler.getValueAsInt(1), disassembler.getValueAsInt(2),
							item));
					...
					result.add(new LongTesterSummaryItem(disassembler.getValueAsInt(1), disassembler.getValueAsInt(0),
							disassembler.getValueAsInt(2), disassembler.getValueAsString(3).charAt(0) == '1', Integer
									.parseInt(disassembler.getValueAsString(3).substring(1)), item));
				} else {
	...
	/**
	 * Returns the number of pending invocations owned by this Tester.
	 * <p>
	 * 
	 * This value includes submission tests, submission system tests, solution
	 * tests, and score calculation invocations.
	 * 
	 * @return The number of pending invocations
	 * @throws LongTesterException
	 *             If the number of pending invocations could not be obtained
	 */
	 ...
	 * @throws LongTesterException
	 *             If the solution could not be stored
	 */
	 ...
	 * @throws LongTesterException
	 *             If solution storage check could not be performed
	 */
	// public boolean isStoredPrimarySolution(int roundId) throws
	// LongTesterException {
	// HierarchicalIdBuilder prefix =
	// buildRoundSystemTestRelatedPrefix(roundId);
	// try {
	// return
	// getInvoker().countSharedObjects(buildSolutionId(prefix)).intValue() == 1;
	// } catch (Exception e) {
	// throw new LongTesterException(e);
	// }
	// }
	 ...
	 * @param args
	 *            The args for the test case
	 * @throws LongTesterException
	 *             If the test case could not be stored
	 */
	...
	/**
	 * Returns true if the test case have been stored for the round has been
	 * stored
	 * 
	 * @param roundId
	 *            The id of the round
	 ...
	 * @return true if stored
	 * @throws LongTesterException
	 *             If test case storage check could not be performed
	 */
	// public boolean isStoredTestCase(int roundId, long testCaseId) throws
	// LongTesterException {
	// HierarchicalIdBuilder prefix =
	// buildRoundSystemTestRelatedPrefix(roundId);
	// try {
	// return getInvoker().countSharedObjects(buildTestCaseId(prefix,
	// testCaseId)).intValue() == 1;
	// } catch (Exception e) {
	// throw new LongTesterException(e);
	// }
	// }
...
//		FarmFactory.getInstance().releaseInvoker(getTesterName(testerId));
	...
	/**
	 * Ensures that the connection to the farm is made, to start listening for
	 * responses to previous invocations
	 * 
	 * @throws LongTesterException
	 *             if the connection to the farm could not be established.
	 */
	 ...
	 * 
	 * @param id
	 *            the submission id of long problem.
	 * @param componentFiles
	 *            the component files.
	 * @param testCaseIds
	 *            the test case ids.
	 ...
	 */
	private void doTestsOnSubmission(LongSubmissionId id, ComponentFiles componentFiles, long[] testCaseIds,
			int roundType, ProblemCustomSettings custom, int testAction, Solution solution,
			HierarchicalIdBuilder context, int priorityCorrection) throws LongTesterException {

		// InvocationRequirements requirements = buildRequeriments(roundType,
		// custom.getMemLimit(), componentFiles.getLanguageId());
		...
		HierarchicalIdBuilder requestIdBuilder = buildRequestIdPrefix(
				buildRoundAndActionPrefix(id.getRoundId(), testAction), id);
		try {
			ArrayList<MarathonCodeTestRequest> requests = new ArrayList<MarathonCodeTestRequest>(testCaseIds.length);
			...
			for (int i = 0; i < testCaseIds.length; i++) {
				...
				// longTest.setComponentFiles(componentFiles);
				// longTest.setArguments(new Object[]{ buildTestCaseId(context,
				// testCaseId) });
				// longTest.setSolution(solution);

				longTest.setArguments(new Object[] { Long.toString(testCaseId) });

				LongTestId attachId = new LongTestId(FarmLongTestRequest.CODE_TYPE_SUBMISSION, id, testCaseId,
						testAction);

				MarathonCodeTestRequest req = new MarathonCodeTestRequest(longTest, componentFiles, solution);
				// req.setInvocationData(attachId);
				requests.add(req);

				// Invocation invocation = new LongTestInvocation(longTest);
				InvocationRequest request = new InvocationRequest(requestIdBuilder.buildId(TYPE_TEST, testCaseId),
						attachId, null, null);
				InvocationData data = controller.storeInvocationRequest(testerId, request);

				queueTestRequest(req, data.getId().toString(), attachId, id.getRoundId(),
						CodeUtil.toLanguageType(componentFiles.getLanguageId()));
	...
	private void doCalculateScores(int roundId, Object id, Solution solution, LongRoundScores longTestResults,
			int testAction, int roundType) throws LongTesterException {
		// R{roundId}.A99.B{testAction}.I{resultGenerationId}..
//		String requestId = buildRoundAndActionPrefix(roundId, ACTION_SCORE).add(TYPE_ACTION_GENERATING_REQUEST,
//				testAction).buildId(TYPE_RESULT_ID, longTestResults.getResultGenerationId());
...
//			InvocationRequest request = new InvocationRequest(requestId, ,
//					null, null);
//			request.setPriority(InvocationRequest.PRIORITY_HIGH);

			MarathonCodeScoringRequest req = new MarathonCodeScoringRequest(solution, longTestResults);
			
			CodeProcessingRequest cpr = new CodeProcessingRequest(req, "", AppType.MARATHON,
					ActionType.SCORE, LanguageType.JAVA, "marathonCodeScorer",
					MarathonCodeProcessingResultHandler.class.getName(), roundId, new LongTestScoreId(testAction, id));
	...
	private void storeTestCase(Object[] objects, String testCaseKey) throws LongTesterException {
		try {
			controller.storeSharedObject(testerId, testCaseKey, objects);
		} catch (Exception e) {
			throw new LongTesterException(e);
		}
	}

//	private void storeComponentFiles(ComponentFiles componentFiles, String componentFilesKey)
//			throws DuplicatedIdentifierException, NotAllowedToRegisterException, FarmException {
//		log.warn("Ignoring component files storage call for " + componentFilesKey);
...
//	}
	...
	private void storePrimarySolution(Solution solution, String solutionKey) throws LongTesterException {
		try {
			controller.storeSharedObject(testerId, solutionKey, solution);
		} catch (Exception e) {
			throw new LongTesterException(e);
		}
	}
	 ...
	 * 
	 * @param roundType
	 *            the round type used to select test processor.
...
//	}
	...
	// private FarmInvoker getInvoker() throws NotAllowedToRegisterException,
	// FarmException {
...
//	}
	...
	private HierarchicalIdBuilder buildGroupRelatedPrefix(long testGroupId) {
		return new HierarchicalIdBuilder().add(TYPE_ACTION, ServicesConstants.MPSQAS_TEST_ACTION)
				.add(TYPE_ACTION, ServicesConstants.MPSQAS_TEST_ACTION).add(TYPE_GROUP, testGroupId)
	...
	private HierarchicalIdBuilder buildRequestIdPrefix(HierarchicalIdBuilder context, LongSubmissionId id) {
		return buildRequestIdPrefix(context, id.getCoderId()).add(TYPE_SUBMISSION,
				(id.isExample() ? "1" : "0") + id.getSubmissionNumber());
	}

	private HierarchicalIdBuilder buildRequestIdPrefix(HierarchicalIdBuilder context, long coderId) {
		return new HierarchicalIdBuilder(context).add(TYPE_CODER, coderId);
	}

	private HierarchicalIdBuilder buildUserTestRelatedPrefix(int roundId, long coderId) {
		return buildRoundAndActionPrefix(roundId, ServicesConstants.LONG_TEST_ACTION).add(TYPE_CODER, coderId);
	}
	...
	private HierarchicalIdBuilder buildRoundRelatedPrefix(int roundId) {
		return new HierarchicalIdBuilder().add(TYPE_ROUND, roundId);
	}
...
//	}
	...
	/**
	 * LongTestInvoker results handler.
	 * <p>
	 * 
	 * Implementators of this interface are responsible for handling responses
	 * for test results.
	 * <p>
	 * 
	 * NOTE: Results for the same submission and test case id might arrive more
	 * than one time.
	 */
	public static interface LongTesterHandler {
		/**
		 * This method is called every time a result for a long test case is
		 * ready.
		 * 
		 * @param id
		 *            The id of the tested submission
		 * @param testCaseId
		 *            The id of the test case
		 * @param testResults
		...
		/**
		 * This method is called every time a result for a long system test case
		 * is ready.
		 * 
		 * @param id
		 *            The id of the tested submission
		...
		/**
		 * This method is called every time a result for a mpsqas long test case
		 * is ready.
		 * 
		...
		/**
		 * This method is called every time a result for an score calculation
		 * invoked using
		 * {@link LongTesterInvoker#calculateTestScores(LongSubmissionId, Solution, LongRoundScores)}
		 * is ready.
		 * 
		 * @param id
		 *            The long submission id for which the score calculation was
		 *            made
		 * @param lrr
		 *            The LongRoundScores produced as a result of the
		 *            calculation
		 * @return true if the result was handled successfully
		 */
		 ...
		 * This method is called every time a result for an score calculation
		 * invoked using
		 * {@link LongTesterInvoker#calculateSystemTestScores(int, Solution, LongRoundScores)}
		 * is ready.
		 * 
		 * @param roundId
		 *            The id of the round for which the score calculation was
		 *            made
		 * @param lrr
		 *            The LongRoundScores produced as a result of the
		 *            calculation

 ...
 *
 * <p>
 * Changes in version 1.6 (TopCoder Competiton Engine - Automatically End Matches v1.0) :
 * <ol>
 *      <li>Update {@link #GET_COMPETITION_ROUND_QUERY} field.</li>
 *      <li>Update {@link #getCompetitionRound(Connection conn, int roundID)} method.</li>
 * </ol>
 * </p>
 *
 * <p>
 * Changes in version 1.7 (Return Time Infos When Opening Problems v1.0) :
 * <ol>
 *      <li>Add {@link #GET_CODER_COMPONENT_ACTIVITY} field.</li>
 *      <li>Add {@link #getCoderComponentActivity(int componentId, int roundId, int coderId)} method.</li>
 * </ol>
 * </p>
 *
 * <p>
 * Changes in version 1.8 (TopCoder Competition Engine - Add Match Super User Role v1.0) :
 * <ol>
 *      <li>Update getUser(Connection conn, int userID, boolean activeOnly) method to support web admin role.</li>
 *      <li>Update resultSetToUser method to support web admin role.</li>
 * </ol>
 * </p>
 *
 * <p>
 * Changes for 1.9 (TopCoder Competition Engine - Return Submission Information)
 * <ol>
 *     <li>Update {@link #GET_CODER_COMPONENT_ACTIVITY} field to return 'open_time', 'submission_count', 'score' info</li>
 *     <li>Update {@link #getCoderComponentActivity(int componentId, int roundId, int coderId)} method to
 *         set the 'openTime', 'submissionCount', 'score' info </li>
 * </ol>
 * </p>
 * @author savon_cn, freegod, xjtufreeman, TCSASSEMBLER
 * @version 1.9
 */
...
//    private static final FastDateFormat DATE_FORMAT_HH_MM = FastDateFormat.getInstance("HH:mm");
    ...
    private static final String GET_COMPETITION_ROUND_QUERY = "SELECT" + "  c.contest_id," + "  c.name,"
        ...
        + "  ra.p, " + "  r.region_id, " + "  c.season_id, " + "r.auto_end " + " FROM contest c, round r "
    ...
    /**
     * Get the coder component activity
     * @since 1.6
     */
    private final static String GET_CODER_COMPONENT_ACTIVITY = "select first 1 a.*, sub.submit_time from (" +
                "select cl.open_time, cl.saved_time, cl.compiled_time, cs.submission_number as submission_count, cs.points as score, cs.component_state_id from component_state cs, compilation cl" +
                " where cl.component_state_id=cs.component_state_id and cs.component_id=? and cs.round_id=? and cs.coder_id=?) a" +
                " left outer join submission sub on a.component_state_id = sub.component_state_id order by submit_time desc";
...
//        + "group by r_reg.coder_id";
    ...
    /**
     * The SQL for get the user.
     * Changes in 1.8: add new return field is_web_admin.
     */
    private final static String GET_USER_SQL = "SELECT u.user_id, u.handle, cr.rating, c.language_id, cr.num_ratings, c.member_since, u.last_login,  "
        ...
        + "(select count(gu2.group_id) from group_user gu2 where gu2.user_id = u.user_id and gu2.group_id = ?) as is_web_admin, "
        + "u.status "
        + "FROM user u, coder c, OUTER(algo_rating cr, OUTER(round_segment rs)), OUTER(user_address_xref uax, address a, country co2), OUTER(country co), OUTER(email e),  "
        + "coder_type ct, OUTER(current_school cs, school s) , OUTER(algo_rating cr_hs, OUTER(round_segment rs_hs)), "
                ...
                try {
                    ...
                    case ContestConstants.LONG_ANSWER:
                        if (allAnswers.isEmpty()) {
                            //  if question does not have predefined answers then use 0 as answerID
                            recordResponse(conn, coderId, questionID, 0, (String) answers.get(0));
                        }
                        else {
                            // otherwise get id of the first choice
...
//    }
     ...
     */
    public CoderComponentActivity getCoderComponentActivity(int componentId, int roundId, int coderId)
            throws DBServicesException {
        debug("getCoderComponentActivity: componentId = " + componentId + ", roundId = " + roundId
                + ",coderId" + coderId);

        PreparedStatement ps = null;
        ResultSet rs = null;
        Connection conn = null;
        CoderComponentActivity activity = new CoderComponentActivity(componentId, roundId, coderId);
        try {
            conn = DBMS.getConnection();
            ps = conn.prepareStatement(GET_CODER_COMPONENT_ACTIVITY);
            ...
            if (rs.next()) {
                activity.setLastSavedTime(rs.getLong("saved_time"));
                activity.setLastCompiledTime(rs.getLong("compiled_time"));
                activity.setLastSubmitTime(rs.getLong("submit_time"));
                activity.setOpenTime(rs.getLong("open_time"));
                activity.setSubmissionCount(rs.getInt("submission_count"));
     ...
     * <p>
     *  Changes in 1.8: add new parameter checkAdmin4Web to show if fetching the admin for web field is required.
     * </p>
     * @param rs the result set
     * @param newRatingMap the new rating map
     * @param newHSRatingMap the new high school rating map
     * @param newMMRatingMap the new MM rating map
     * @param isLevelOneAdminMap the map storing is_level_one properties
     * @param isLevelTwoAdminMap the map storing is_level_two properties
     * @param checkAdmin4Web if check admin for web is required
     * @return the user found from the backend db
    ...
    private User resultSetToUser(ResultSet rs, Map<Integer, Integer> newRatingMap,
        ...
        Map<Integer, Integer> isLevelOneAdminMap, Map<Integer, Integer> isLevelTwoAdminMap, boolean checkAdmin4Web)
        throws SQLException {
        if (rs.next()) {
            ...
            boolean isAdmin4Web = checkAdmin4Web ? rs.getInt("is_web_admin") != 0 : false;
...
//    }
     ...
     * Get the users list.
     * <p>
     *  Changes in 1.8: change is made due to method resultSetToUser is changed.
     * </p>
     ...
     * @throws DisabledUserDBException is the user is disabled
     */
     ...
     * <p>
     *  Changes in 1.8: add support to fetching the admin for web field.
     * </p>
     * @param conn the connection instance
     * @param userID the user id
     * @param activeOnly if fetching active user only
     * @return the user found
     * @throws SQLException if any sql error
     * @throws DBServicesException if any db service exception happens
     */
    private User getUser(Connection conn, int userID, boolean activeOnly)
        ...
        try {
            ...
            ps.setInt(3, ServerContestConstants.WEB_ADMIN_GROUP_ID);
            ps.setInt(4, userID);
            rs = ps.executeQuery();
            u = resultSetToUser(rs, null, null, null, null, null, true);

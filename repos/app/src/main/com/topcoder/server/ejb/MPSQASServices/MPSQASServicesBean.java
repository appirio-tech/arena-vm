   ...
   /**
    ...
    private ArrayList compileSolution(
        ...
        //get the solution id
            ...
            try {
				mfiles = executeMPSQASRequest(mfiles, userId, ActionType.COMPILE, "mpsqasCompiler",
						custom.getCompileTimeLimit(), TimeUnit.SECONDS);
			} catch (Exception e) {
	...
	private MPSQASFiles executeMPSQASRequest(MPSQASFiles mfiles, long userId, ActionType actionType,
			String processorName, int timeout, TimeUnit toUnit) throws InterruptedException, ExecutionException,
			TimeoutException {
		
		HierarchicalIdBuilder idBuilder = new HierarchicalIdBuilder();
		String clientName;
		if (actionType.equals(ActionType.TEST)) {
			clientName = "MPSQASTest-MPSQAS";
			idBuilder.add(TYPE_ACTION, ServicesConstants.MPSQAS_TEST_ACTION);
			idBuilder.add(TYPE_SOLUTION, mfiles.getSolutionId());
		} else {
			clientName = "MPSQASTest-Compiler";
			idBuilder.add(TYPE_SOLUTION, mfiles.getSolutionId());
		}
		
		getCodeService().deletePendingCodeRequests(clientName, new CodeProcessingRequestMetadata(AppType.ADMIN,
				actionType, CodeUtil.toLanguageType(mfiles.getLanguage()), idBuilder.getId(), null));
		
		CodeProcessingRequest cpr = new CodeProcessingRequest(mfiles, idBuilder.getId(), AppType.ADMIN,
				actionType, CodeUtil.toLanguageType(mfiles.getLanguage()), processorName, null, null, null);
		cpr.getMetadata().setSynchronous(true);
		cpr.getMetadata().setSyncTimeout(Long.valueOf(toUnit.toSeconds(timeout)).intValue());
		Future<CodeProcessingResult> resultPromise = getCodeService().sendToProcessor(cpr);
		CodeProcessingResult result = resultPromise.get(timeout, toUnit);
		if (result == null) {
			throw new TimeoutException();
		}
		return (MPSQASFiles)result.getResultData();
	}
     ...
     */
    private String test(Object[] args, int componentId, int userId, int type,
            Connection conn) throws Exception {
        PreparedStatement ps = null;
        StringBuilder testResults = new StringBuilder(256);
        ResultSet rs = null;
                        ...
                        if (componentType == ProblemConstants.LONG_COMPONENT && primary == 0) {
                            ...
                            if (rs2.next()) {
                                ...
                                if (primary == ApplicationConstants.PRIMARY_SOLUTION) {
                                    ...
                                    resultmfiles = executeMPSQASRequest(mfiles, userId, ActionType.TEST, "mpsqasTester", 12, TimeUnit.HOURS);
                                ...
                                // TODO: figure out what maximum test length is
                                resultmfiles = executeMPSQASRequest(mfiles, userId, ActionType.TEST, "mpsqasTester", 12, TimeUnit.HOURS);
    ...
    private String buildOverallScoreMessage(int userId, Connection cnn) {
        ...
        try {
            ...
            MPSQASFiles result = runMPSQASTest(userId, solutionId, componentId, "score", new Object[] {scores}, cnn);
     ...
     * the tester returns.
     */
    public MPSQASFiles test(Object[] args, int userId, int componentId, int solutionId,
            String methodName) {
        Connection conn = null;
        try {
            conn = DBMS.getConnection();
            return test(args, userId, componentId, solutionId, methodName, conn);
        } catch (Exception e) {
     ...
     */
    private MPSQASFiles test(Object[] args, int userId, int componentId, int solutionId,
            ...
            if (argTypes.size() == args.length) {
                ...
                if (rs.next()) {
                    ...
                    if (rs2.next()) {
                        ...
                        mfiles.setComponentType(componentType);
                        mfiles = executeMPSQASRequest(mfiles, userId, ActionType.TEST, "mpsqasTester", 12, TimeUnit.HOURS);
                    } else {
                        testResults.append("(no compiled files to test)");
                    }
                } else {
     ...
     */
    public MPSQASFiles[] test(Object[][] args, int userId, int componentId,
     ...
     * @return the mpsqas file with test result.
     */
    private MPSQASFiles[] test(Object[][] args, int userId, int componentId,
            ...
            if (rs.next()) {
                ...
                if (rs2.next()) {
                    ...
                    for (int i = 0; i < args.length; i++) {
                        ...
                        mfiles.setArgVals(argValues);
                        try {
                            mfilesArray[i] = executeMPSQASRequest(mfiles, userId, ActionType.TEST, "mpsqasTester", 12, TimeUnit.HOURS);
                        } catch (Exception e) {
                            s_trace.error("Error performing test.", e);
            ...
            for (i = 0; i < args.size(); i++) {
                ...
                for (j = 0; j < mpsqasFiles.length; j++) {
                    mpsqasFiles[j].setArgVals(new ArrayList(Arrays.asList(
                            (Object[]) args.get(i))));
                    // don't know the user id here
                    mfiles = executeMPSQASRequest(mpsqasFiles[j], 0, ActionType.TEST, "mpsqasTester", 12, TimeUnit.HOURS);
...
//            ps.close();
     ...
     */
    private MPSQASFiles runMPSQASTest(int userId, int solutionId, int componentId,
                String methodName, Object[] args, Connection conn) throws SQLException, InterruptedException, ExecutionException, TimeoutException  {
        ...
        return executeMPSQASRequest(mfiles, userId, ActionType.TEST, "mpsqasTester", 12, TimeUnit.HOURS);
    }

 ...
 *
 * <p>
 * Changes in version 1.4 (TopCoder Competiton Engine - Automatically End Matches v1.0) :
 * <ol>
 *      <li>Update {@link #sendPhaseEvent(Iterator connectionIDs, PhaseEvent event)} method.</li>
 * </ol>
 * </p>
 *
 * <p>
 * Changes in version 1.5 (Return Time Infos When Opening Problems v1.0) :
 * <ol>
 *      <li>Update {@link #getChallengeComponent(Integer connectionID, BaseCodingRoom room, RoundComponent component,
 *           String code, Coder defenderCoder, Integer languageID, CoderComponentActivity activity)} method.</li>
 *      <li>Update {@link #openComponent(Integer connectionID, int componentID, String code, int editSource, int roomType, int roomID,
 *           Integer languageID, String writerHandler, PhaseDataResponse phaseDataResponse, CoderComponentActivity activity)} method.</li>
 *      <li>Update {@link #getOpenComponentResponse(int componentID, String code, int editSource, int roomType, int roomID,
 *           Integer languageID, String writerHandler, CoderComponentActivity activity)} method.</li>
 * </ol>
 * </p>
 *
 * <p>
 * Changes in version 1.6 (TopCoder Competition Engine - Add Match Super User Role v1.0) :
 * <ol>
 *      <li>Update loginSuccess method for the changed User constructor.</li>
 *      <li>Update updateCurrentUser method for the changed User constructor.</li>
 * </ol>
 * </p>
 *
 * <p>
 * Changes for 1.7 (TopCoder Competition Engine - Return Submission Information)
 * <ol>
 *     <li>Update {@link #getOpenComponentResponse(int componentID, String code, int editSource, int roomType, int roomID,
        Integer languageID, String writerHandler, CoderComponentActivity activity)} method to set the
        'openTime', 'submissionCount', 'score' info </li>
 * </ol>
 * </p>
 * @author Graham Hesselroth, dexy, gondzo, xjtufreeman, TCSASSEMBLER
 * @version 1.7
 */
...
public final class ResponseProcessor {
    ...
    static void createPracticeRoomLists() {
        ...
        for (int i = 0; i < practiceRounds.length; i++) {
            ...
            if (practiceRound != null) {
                ...
                throw new IllegalStateException("CoreServices returned null ContestRound for practice room");
            }
        }
    ...
    /**
     * Sends responses to the user indicating that the login was successful.
     * <p>
     *  Changes in 1.6: change the initialization of the UserInfo instance due to its constructor's change.
     * </p>
     * @param connectionID the connection id
     * @param requestID the request id
     * @param user the user
     * @param autoRegisterForActiveRound if auto register for the active round
     */
    ...
    */
     ...
     * 
     * @param connectionIDs the processor connection id.
    ...
    static void sendPhaseEvent(Iterator connectionIDs, PhaseEvent event) {
        ...
        case ContestConstants.CODING_PHASE:
            systemMessage = "System> " + displayName + " Coding Phase has started.\n";
            userMessage = "Coding Phase has started for " + displayName + ".";
        ...
        case ContestConstants.INTERMISSION_PHASE:
            systemMessage = "System> " + displayName + " Coding Phase has ended.\n";
            userMessage = "The coding phase has ended for " + displayName + ".";
            break;
        case ContestConstants.CHALLENGE_PHASE:
            systemMessage = "System> " + displayName + " Challenge Phase has started.\n";
            userMessage = "Challenge Phase has started for " + displayName + ".";
            break;
        case ContestConstants.VOTING_PHASE:
            systemMessage = "System> " + displayName + " Voting Phase has started.\n";
            userMessage = "Voting Phase has started for " + displayName + ".";
            break;
        case ContestConstants.TIE_BREAKING_VOTING_PHASE:
            systemMessage = "System> " + displayName + " Tie Breaking Voting Phase has started.\n";
            userMessage = "Tie Breaking Voting Phase has started for " + displayName + ".";
            break;
        case ContestConstants.PENDING_SYSTESTS_PHASE:
            ...
            if (contest.getRoundProperties().hasChallengePhase()) {
                systemMessage = "System> " + displayName + " Challenge Phase has ended.\n";
                userMessage = "Challenge Phase has ended for " + displayName + ".";
            } else {
                //More ahhhs!!!!!! Again!
                if (contest.getIntermissionStart().before(contest.getIntermissionEnd())) {
                    systemMessage = "System> " + displayName + " Intermission Phase ended.\n";
                    userMessage = "Intermission Phase has ended for " + displayName + ".";
                } else {
                    systemMessage = "System> " + displayName + " Coding Phase has ended.\n";
                    userMessage = "Coding Phase has ended for " + displayName + ".";
                }
            }
            
            if (contest.isAutoEnd()) {
                contest.setShowAutoSystemTestMessage(true);
            }
            break;
        case ContestConstants.SYSTEM_TESTING_PHASE:
            if (contest.isAutoEnd() && contest.getRoundProperties().hasChallengePhase()) {
                if (contest.isShowAutoSystemTestMessage()) {
                    systemMessage = "System> " + displayName + " System Testing is starting automatically.\n";
                    userMessage = "System Testing is starting automatically for " + displayName + ".\n";
                    //Only need to show once
                    contest.setShowAutoSystemTestMessage(false);    
                } else if (contest.isSystemTestEnd()) {
                    systemMessage = "System> " + displayName + " System Testing is end automatically.\n";
                    userMessage = "System Testing is end automatically for " + displayName + ".\n";
                    contest.setSystemTestEnd(false);
                }
            }
                    ...
                    if (assignedRoomIDInteger != null) {
                        ...
                        if (userRegistered) {
                            if (userNotInRoom) {
                                ...
                                case ContestConstants.SYSTEM_TESTING_PHASE:
                                    if (contest.isAutoEnd()) {
                                        String message1 = userMessage;
                                        String title1 = "Phase Change";
                                        if(event.getPopups())
                                        {
                                            nextUserResponses.add(new PopUpGenericResponse(title1, message1, ContestConstants.GENERIC, ContestConstants.LABEL));
                                        }
                                    }
                                    break;
                                default:
    ...
    static void registerInfo(Integer connectionID, Round contest, Registration registration) {
        if (contest == null) {
    		throw new IllegalArgumentException("contest is required");
    	}
    	
	    long minutesUntilCoding = contest != null && contest.getCodingStart() != null ? 
		    ((contest.getCodingStart().getTime() - System.currentTimeMillis()) / 1000 ) / 60 : 0;
    ...
    /**
     * Get all challenge component
     * @param connectionID the connection id.
     * @param room the base coding room.
     * @param component the component.
     * @param code the coder handler
     * @param defenderCoder the defender coder.
     * @param languageID the language id.
     * @param activity the code component activity
     */
    static void getChallengeComponent(Integer connectionID, BaseCodingRoom room, RoundComponent component, String code,
            Coder defenderCoder, Integer languageID, CoderComponentActivity activity) {
        ...
        allResponses.add(response);
        allResponses.add(getOpenComponentResponse(component.getComponent().getComponentID(), code, ContestConstants.VIEW_SOURCE, room.getType(),
                room.getRoomID(), languageID, defenderCoder.getName(), activity));
        process(connectionID, allResponses);
    }
    ...
    static void getSourceCode(Integer connectionID, BaseCodingRoom room, RoundComponent component, String code,
        ...
        getChallengeComponent(connectionID, room, component, code, defenderCoder, languageID, null);
    }
     ...
     * @param editSource the edit source flag.
     * @param roomType the root type.
     * @param roomID the room id.
     * @param languageID the language id.
     * @param writerHandler the writer handler.
     * @param phaseDataResponse the phase data.
     * @param activity the coder component activity.
     */
    static void openComponent(Integer connectionID, int componentID, String code, int editSource, int roomType, int roomID,
            Integer languageID, String writerHandler, PhaseDataResponse phaseDataResponse, CoderComponentActivity activity) {
        ArrayList allResponses = new ArrayList();
        allResponses.add(getOpenComponentResponse(componentID, code, editSource, roomType, roomID,
                languageID, writerHandler, activity));
     ...
     * @param writerHandler the writer handler.
     * @param activity the coder component activity.
     * @return the open component response.
     */
    static OpenComponentResponse getOpenComponentResponse(int componentID, String code, int editSource, int roomType, int roomID,
            Integer languageID, String writerHandler, CoderComponentActivity activity) {
        OpenComponentResponse response = new OpenComponentResponse(writerHandler, componentID, code, editSource, roomType, roomID,
                languageID.intValue());
        if (activity != null) {
            response.setLastCompiledTime(activity.getLastCompiledTime());
            response.setLastSavedTime(activity.getLastSavedTime());
            response.setLastSubmitTime(activity.getLastSubmitTime());
            response.setOpenTime(activity.getOpenTime());
            response.setSubmissionCount(activity.getSubmissionCount());
            response.setScore(activity.getScore());
        }
        return response;
    }

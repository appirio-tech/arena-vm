...
public class WebSocketDisconnectListener implements DisconnectListener {
	private static final Logger logger = Logger.getLogger(WebSocketDisconnectListener.class);
    ...
    public void onDisconnect(SocketIOClient client) {
        try {
            ...
            logger.info("error processing disconnect request: " + e.getMessage(), e);
            //does not matter, the connection will time out
        } finally {
        	// make sure we clean these up or we'll end up with a leak
        	try {
	        	server.getSessionToConnectionMap().remove(client.getSessionId());
	            server.getSessionToUserHandleMap().remove(client.getSessionId());
        	} catch (Throwable t) {
        		logger.error("Unable to remove client data from memory: " + t.getMessage(), t);
        	}
        }
    }
}

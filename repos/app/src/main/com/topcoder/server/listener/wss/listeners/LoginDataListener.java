...
public class LoginDataListener extends BaseDataListener<LoginRequest> {
	private static final Logger logger = Logger.getLogger(LoginDataListener.class);
    ...
    public void onData(SocketIOClient client, LoginRequest data, AckRequest ackRequest) {
        ...
        if (logger.isInfoEnabled()) {
        	logger.info("Process Login request for user: " + username);
        }
        ...
        try {
            ...
            if (logger.isInfoEnabled()) {
            	logger.info("Login request for user: " + username + " processed.");
            }
        } catch (Exception e) {

...
public class SSOLoginDataListener extends BaseDataListener<SSOLoginRequest> {
	private static final Logger logger = Logger.getLogger(SSOLoginDataListener.class);
...
//        String jwt = data.getJWT();
        ...
        if (logger.isInfoEnabled()) {
        	logger.info("Process SSO Login request for session: " + client.getSessionId() + ", sso: " + sso);
        }
        try {
...
//            }
            ...
            try {
            	...
            	logger.info("Invalid SSO token: " + e.getMessage());
            }
            ...
            if (logger.isInfoEnabled()) {
            	logger.info("Login request for sso: " + sso + " processed.");
            }
        } catch (Exception e) {

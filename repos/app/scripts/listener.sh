...
if [[ $1 != "" ]] ; then
	PORT=$1
	PID_FILE=MainListener.$PORT.pid
	shift
fi
...
if [ "$CMD" = "run" ] ; then
    ...
    echo "nohup $JAVACMD -Dcontestconstants.ACCEPT_MULTIPLE_SUBMISSIONS=true -DVM_INSTANCE_ID=Listener -cp $CP $MAIN_LISTENER_JAVA_OPTS -Dclientsocket.buffersize.25000=524288 -D$LOGGING_PROPERTY=$LOGGING_ID $MAIN $PORT $PROCESSOR $@ 2>&1 &"
    nohup $JAVACMD -Dcontestconstants.ACCEPT_MULTIPLE_SUBMISSIONS=true -DVM_INSTANCE_ID=Listener -cp $CP $MAIN_LISTENER_JAVA_OPTS -Dclientsocket.buffersize.25000=524288 -D$LOGGING_PROPERTY=$LOGGING_ID $MAIN $PORT $PROCESSOR $@ 2>&1 &
    echo $! > $PID_FILE

...
for /F %%V in (%FARM_LAUNCH_PATH%\idfile) do java -XX:+UseConcMarkSweepGC -Xms128m -Xmx640m -cp %FARM_LAUNCH_PATH%\netx-0.5.1.jar netx.jnlp.runtime.Boot -basedir %FARM_LAUNCH_PATH% -noupdate -nosecurity -headless -jnlp http:///farm-controller.topcoder.com:8080/farm-deployer/launch.jnlp?type=controller^&id=%%V
sleep 3
goto loop

...
public final class ChatPanel extends JPanel implements Observer, UserListListener, ChatView {
    ...
    private JButton modeSwitcher = new JButton(NORMALMODELOOK);
    ...
    private LookupJLabelTextField chatInputBox = new LookupJLabelTextField(userModel, true);
    ...
    public ChatPanel(ContestApplet ca, String title, int scope) {
        super(new GridBagLayout());

        this.ca = ca;
        localPref.addSaveObserver(this);
        createChatPanel(title, 0, 0, Common.BG_COLOR);
        ignoreList = LocalPreferences.getInstance().getIgnoreList();
        allowedList = LocalPreferences.getInstance().getAllowedList();
        this.scope = scope;
    }

    public ChatPanel(ContestApplet ca, int scope) {
        this(ca, "Chat Area", scope);
    }

    public ChatPanel(ContestApplet ca) {
        this(ca, ContestConstants.GLOBAL_CHAT_SCOPE);
    }

    public ChatPanel(ContestApplet ca, String title) {
        this(ca, title, ContestConstants.GLOBAL_CHAT_SCOPE);
    }
    ...
    public void setPanelEnabled(boolean on) {
        enabled = on;
        if(enabled) {
            chatInputBox.setEnabled(true);
            chatInputBox.setBackground(Color.white);
        } else {
            chatInputBox.setEnabled(false);
            chatInputBox.setBackground(Color.gray);
        }
    }

    ////////////////////////////////////////////////////////////////////////////////
    private void createChatPanel(String title, int width, int height, Color color)
            ////////////////////////////////////////////////////////////////////////////////
    {

        this.setLayout(new BorderLayout());

        pane = new JScrollPane(chatContainer);
        pane.setBackground(color);
        pane.setBorder(new EmptyBorder(-1, -1, -1, -1));

        chatInputBox.setSelectedTextColor(Common.HF_COLOR);
        chatInputBox.setSelectionColor(Common.HB_COLOR);
        chatInputBox.setMinimumSize(new Dimension(0, 20));
        chatInputBox.setPreferredSize(new Dimension(0, 20));

        modeSwitcher.setFont(new Font("SansSerif", Font.PLAIN, 10));
        modeSwitcher.setMinimumSize(new Dimension(25, 21));
        modeSwitcher.setPreferredSize(new Dimension(25, 21));
        modeSwitcher.setMargin(new Insets(-1, -1, -1, -1));
        modeSwitcher.addActionListener(new ModeSwitchHandler());
        modeSwitcher.setBackground(Common.WPB_COLOR);
        modeSwitcher.setForeground(Common.FG_COLOR);
        modeSwitcher.setBorder(BorderFactory.createBevelBorder(BevelBorder.RAISED, Common.FG_COLOR, Common.WPB_COLOR));

        msgType.setFont(new Font("SansSerif", Font.PLAIN, 12));
        msgType.setMinimumSize(new Dimension(0, 20));
        msgType.setPreferredSize(new Dimension(80, 20));
        msgType.setModel(new SortedComboBoxModel());
        //msgType.setBackground(Common.BG_COLOR);
        msgType.addItem(PRIVATEMSG);
        msgType.addItem(ADMINMSG);
        msgType.addItem(GENERALMSG);
        msgType.addItem(MEMSG);
        msgType.addItem(REPLYTOMSG);
        msgType.addItem(MODERATOR_MSG);
        //msgType.setKeySelectionManager(new LookupComboBoxKeyManager(msgType, false));
        msgType.addActionListener(new TypeSwitchHandler());
        msgType.setSelectedItem(GENERALMSG);

        toWho.setFont(new Font("SansSerif", Font.PLAIN, 12));
        toWho.setMinimumSize(new Dimension(0, 20));
        toWho.setPreferredSize(new Dimension(120, 20));
        toWho.setModel(userModel);
        toWho.setSelectedItem(new JLabel(""));
        toWho.setRenderer(new JLabelListCellRenderer());
        toWho.setEditable(true);
        toWho.setEditor(new LookupJLabelComboBoxEditor(userModel));
        ...
        chatContainer.setSelectedTextColor(Common.HF_COLOR);
        chatContainer.setSelectionColor(Common.HB_COLOR);
        //chatContainer.setBackground(Color.black);
        chatContainer.setBackground(localPref.getColor(LocalPreferences.CHATPANELBACK));
        chatContainer.setForeground(Color.green);
        chatContainer.addMouseListener(new MouseHandler());
        ...
        chatContainer.addKeyListener(keyHandler);
        chatInputBox.addKeyListener(keyHandler);

        //ci_al = new al("actionPerformed", "chatinputEvent", this);
        ci_al = new ActionListener() {
        ...
        boxPanel.setBackground(Common.BG_COLOR);
        
        Box inputModeBox = Box.createHorizontalBox();
        inputModeBox.add(chatModeButton);
        inputModeBox.add(findModeButton);
        inputModePanel.add(inputModeBox, BorderLayout.CENTER);
        inputModePanel.setBackground(Common.BG_COLOR);
        ...
        findModeButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                setInputMode(FINDMODE);
            }
        });
        
        findMatchCase.addItemListener(new ItemListener() {
            public void itemStateChanged(ItemEvent e) {
                ...
                find(findInput.getText(), false);
            }
        });
        
        findHighlight.addItemListener(new ItemListener() {
            public void itemStateChanged(ItemEvent e) {
                if (e.getStateChange() == ItemEvent.SELECTED) {
        ...
        findInput.addKeyListener(new KeyListener() {
            private String last = "";
            public void keyReleased(KeyEvent e) {
                String current = findInput.getText();
        ...
        applyButtonStyle(findNext);
        applyButtonStyle(findPrevious);
        applyCheckBoxStyle(findMatchCase, 78);
        applyCheckBoxStyle(findHighlight, 68);
        
        chatModeButton.setMargin(new Insets(0,0,0,0));
        findModeButton.setMargin(new Insets(0,0,0,0));
        chatModeButton.setPreferredSize(new Dimension(81, 18));
        findModeButton.setPreferredSize(new Dimension(81, 18));
        chatModeButton.setBorder(new EmptyBorder(0,0,0,0));
        findModeButton.setBorder(new EmptyBorder(0,0,0,0));
        
        Box findPanelBox = Box.createHorizontalBox();
        findPanelBox.add(findInput);
        findPanelBox.add(Box.createHorizontalStrut(5));
        findPanelBox.add(findNext);
        findPanelBox.add(Box.createHorizontalStrut(3));
        findPanelBox.add(findMatchCase);
        findPanelBox.add(findHighlight);
        findPanel.setBackground(Common.BG_COLOR);
        findPanel.add(findPanelBox, BorderLayout.CENTER);
        
        add(pane, BorderLayout.CENTER); // center
    ...
    private void applyButtonStyle(JButton button) {
        ...
        button.setMinimumSize(new Dimension(80, 21));
        button.setPreferredSize(new Dimension(80, 21));
        button.setBackground(Common.WPB_COLOR);
        button.setForeground(Common.FG_COLOR);
    }
    
    private void applyCheckBoxStyle(JCheckBox checkBox, int width) {
        checkBox.setFont(new Font("SansSerif", Font.PLAIN, 10));
        checkBox.setMinimumSize(new Dimension(width, 21));
        checkBox.setPreferredSize(new Dimension(width, 21));
        checkBox.setMargin(new Insets(0,0,0,0));
        checkBox.setBackground(Common.BG_COLOR);
        checkBox.setForeground(Common.FG_COLOR);
    }
            ...
            ////////////////////////////////////////////////////////////////////////////////
    {
        ...
        chatInputBox.removeActionListener(ci_al);
        localPref.removeSaveObserver(this);
        localPref.addSaveObserver(this);
        chatInputBox.addActionListener(ci_al);
            ...
            ////////////////////////////////////////////////////////////////////////////////
    {
        ...
        try {
            ...
            StyledDocument doc = chatContainer.getStyledDocument();
   ...
   //         }
	...
	private boolean isAtLastLine() {
		...
		return (bar.getValue() + bar.getVisibleAmount() + chatContainer.getFontMetrics(font).getHeight() >= bar.getMaximum());
	}
    ...
    private void setCaretPosition() {
        if (!ca.isScrollingChatDisabled()) {
            int length = chatContainer.getDocument().getLength();
            chatContainer.setCaretPosition(length);
        }
    }
    ...
    {   
        ...
        chatInputBox.setFont(new Font(localPref.getFont(LocalPreferences.CHATFONT), Font.PLAIN, localPref.getFontSize(LocalPreferences.CHATFONTSIZE)));
            ...
            ////////////////////////////////////////////////////////////////////////////////
    {
        ...
        if (chatInputBox.getText().length() > 256) {
            Common.showMessage("Error", "You have entered " +
                    chatInputBox.getText().length() +
        ...
        // Get the message
        String theMsg = chatInputBox.getText();
...
/*	    if(theMsg.startsWith("setHighlight")) {
        ...
        JLabel label = (JLabel) toWho.getSelectedItem();
        ...
        boolean overrideEnhanced = theMsg.startsWith(MSG) || theMsg.startsWith(ME) || theMsg.startsWith(ADMINS) || theMsg.startsWith(ASK) || (theMsg.indexOf(":") > 0 && theMsg.substring(0, theMsg.indexOf(":")).indexOf(" ") < 0 && !theMsg.startsWith("http:"));
        ...
        if (!ca.isAutoEnhancedChatDisabled() && (currentMode == NORMALMODE || currentMode == ENHANCEDMODE)) {
            ...
            if (textMessage.startsWith(ME)) {
                ...
                toWho.setSelectedItem(copyLabel);
                ...
                if (userName.indexOf(" ") < 0 && !userName.equals("http")) {
                    setMode(ENHANCEDMODE);
                    msgType.setSelectedItem(REPLYTOMSG);
                ...
                setMode(ENHANCEDMODE);
                msgType.setSelectedItem(MODERATOR_MSG);
            } else {
    ...
    private void setupTabs() {
        ...
        bottomPanel.remove(inputModePanel);
        if (!ca.isChatFindTabsDisabled()) {
        	bottomPanel.add(inputModePanel, BorderLayout.NORTH);
        }
        else {
    ...
    private void setInputMode(int mode) {
        ...
        if (currentInputPanel != null) bottomPanel.remove(currentInputPanel);
        
        remove(bottomPanel); // in case of read-only, don't display
        
        String chatImage = "chat-off.gif";
        String findImage = "find-off.gif";
        Component focus;
        ...
        case FINDMODE:
            ...
            findImage = "find-on.gif";
            focus = findInput;
            break;
        case CHATMODE:
        default:
            currentInputPanel = boxPanel;
            chatImage = "chat-on.gif";
            focus = chatInputBox;
            break;
        }
        
        chatModeButton.setIcon(Common.getImage(chatImage, ca));
        findModeButton.setIcon(Common.getImage(findImage, ca));
        
        bottomPanel.add(currentInputPanel, BorderLayout.SOUTH);
    ...
    private void setMode(int mode) {
        ...
        // Remove the currentBox
        if (currentBox != null) boxPanel.remove(currentBox);
        ...
        switch (mode) {
        case NORMALMODE:
            modeSwitcher.setText(NORMALMODELOOK);
            currentBox.add(modeSwitcher);
            currentBox.add(Box.createRigidArea(spacing));
            currentBox.add(chatInputBox);
            focusTo = chatInputBox;
            break;

        case ENHANCEDMODE:
            modeSwitcher.setText(ENHANCEDMODELOOK);
            currentBox.add(modeSwitcher);
            currentBox.add(Box.createRigidArea(spacing));
            currentBox.add(msgType);
            currentBox.add(Box.createRigidArea(spacing));
            currentBox.add(toWho);
            currentBox.add(Box.createRigidArea(spacing));
            currentBox.add(chatInputBox);
            focusTo = msgType;
            break;
        ...
        // Put the new box back into the panel
        boxPanel.add(currentBox, BorderLayout.SOUTH);
    ...
    private class ModeSwitchHandler implements ActionListener {
    ...
    private class TypeSwitchHandler implements ActionListener {

        public void actionPerformed(ActionEvent e) {
            String type = (String) msgType.getSelectedItem();
            if (type.equals(GENERALMSG) || type.equals(MEMSG) || type.equals(ADMINMSG)) {
                toWho.setEnabled(false);
                toWho.setSelectedItem(new JLabel(""));
            } else {
                toWho.setEnabled(true);
            }
        }
    }
    ...
    private class MouseHandler extends MouseAdapter {
        ...
        public void mouseClicked(MouseEvent e) {
            ...
            if (e.getClickCount() == 2 && currentMode != HIDDENMODE) {
                int pos = chatContainer.viewToModel(new Point(e.getX(), e.getY()));
                Element elm = chatContainer.getStyledDocument().getCharacterElement(pos);
                /* Da Twink Daddy - 05/09/2002 - Name Update */
                ...
                if (userName != null) {
                    ...
                    if (ca.isAutoEnhancedChatDisabled()) {
                        ...
                        chatInputBox.setText(temp.toString());
    ...
    private void removeHighlights() {
        Highlighter highlighter = chatContainer.getHighlighter();
    ...
    private String getChatText() {
        Document document = chatContainer.getDocument();
    ...
    private void find(String string, boolean findNext) {
        ...
        if (index >= 0) {
            ...
            chatContainer.setCaretPosition(index);
            findIndex = index;
        }
        else {
            findIndex = 0;

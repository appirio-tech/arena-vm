#       Arena Java Web Start Setup Script
#       Version 1.0.0
...
#
#       This scripts will properly unpack the built arena client zip files
...
ZIP_FILE="/home/apps/dev/comp-eng/arena-client/build/arena-client-*.zip"
ZIP_FILE_NAME=`find $ZIP_FILE`
TARGET_DIR="$APACHE_DIR/classes/7.0"
...
echo "Starting setup for Arena JNLP..."
mkdir -p $APACHE_DIR || die_on_error

if [[ -f $ZIP_FILE_NAME ]]; then
    cp -f $ZIP_FILE $APACHE_DIR/arena-client.zip
    rm -rf $TARGET_DIR
    mkdir -p $TARGET_DIR
...
echo "Done with setup for Arena JNLP."

        ...
        for (int i=0; i<code.length(); i++) {
            if (code.charAt(i)=='{') {
                ...
                if (ok && !currentLocation.elementAt(currentLocation.size()-1).equals("inMethod")) {
                    boolean isClass=(currentLocation.elementAt(currentLocation.size()-1).equals("nothing") || code.lastIndexOf("class ",i)>code.lastIndexOf("(",i));
                    
                    if (isClass)		//start of a class
                    {
                        int classStart2=code.lastIndexOf("class ",i);
                    ...
                    else			//start of a method
                    {
                        ...
                        //attempt to fix exceptions on methodName (...)
...
/////////////////Find new classes///////////////////
    ...
    public int countAutoCodeInOriginalCode() {
        String[] auto={"// Powered by FileEdit","// Powered by CodeProcessor","// Powered by PopsEdit","// Powered by [KawigiEdit]"};
        int rightBrace=originalCode.lastIndexOf("}");
        
        int count=0;
        for (int i=0; i<auto.length; i++) {
            int autoIndex=originalCode.lastIndexOf(auto[i]);
            if (autoIndex>rightBrace) count+=(auto[i].length()-3);		//-3 spaces
        }
    ...
    public int countImports() {
        ...
        while(true) {
            int importStart=code.indexOf("import java",cur);
